...
async def leave_participants(activity_id : int, current_user : User = Depends(get_current_user), db : AsyncSession = Depends(get_db)):
...
async def remove_participant(activity_id : int, user_id : int, current_user : User = Depends(get_current_user), db : AsyncSession = Depends(get_db)):
